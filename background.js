...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "start_keepalive") {
    ...
    // Create an alarm to fire every 30 minutes
    chrome.alarms.create("rvce_keepalive", { periodInMinutes: 30 });
    console.log(
      "[RVCE Auto-Login] Keepalive alarm scheduled for every 30 minutes.",
    );
  }
});
    ...
    chrome.storage.local.get(["rvce_keepalive_url"], (result) => {
      ...
      if (keepaliveUrl) {
        ...
        fetch(keepaliveUrl, { mode: "no-cors" })
          .then(() =>
            console.log("[RVCE Auto-Login] Keepalive ping successful."),
          )
          .catch((err) =>
            console.error("[RVCE Auto-Login] Keepalive ping failed:", err),
          );
      } else {
        console.log(
          "[RVCE Auto-Login] Keepalive URL not found. Clearing alarm.",
        );
